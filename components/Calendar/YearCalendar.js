...
const isDateInRange = (start_date, end_date, date) => {
  ...
  const endDate = end_date ? dayjs(end_date) : startDate;
...
const isValidUrl = (string) => {
  ...
  try {
    ...
    // If it fails, check if it's a proper URL with protocol or www
    return (
...
const YearCalendar = ({ calendarData = [], selectedYear, setSelectedYear }) => {
  ...
  // Handle null/undefined calendarData
          ...
          >
            ...
            <div className="flex-grow pl-0 md:pl-6">
              ...
              <div className="space-y-2">
                ...
                {event?.details?.supporting_sponsors &&
                  Array.isArray(event.details.supporting_sponsors) &&
                  event.details.supporting_sponsors.length > 0 && (
                    <p>
                      <span className="font-semibold mr-2 text-lightgray">
                        Supporting Sponsors:
                      </span>
                      <span className="text-lightgray">
                        {event.details.supporting_sponsors.join(", ")}
                      </span>
                    </p>
                  )}
